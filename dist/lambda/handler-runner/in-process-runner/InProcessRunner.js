...
class InProcessRunner {
  ...
  async run(event, context) {
    ...
    Object.assign(process.env, _classPrivateFieldLooseBase(this, _env)[_env]);
    ...
    } = await (specifier => new Promise(r => r(`${specifier}`)).then(s => _interopRequireWildcard(require(s))))(_classPrivateFieldLooseBase(this, _handlerPath)[_handlerPath]);
    ...
    const executionTimeout = _perf_hooks.performance.now() + _classPrivateFieldLooseBase(this, _timeout)[_timeout];
    ...
    const lambdaContext = {
      ...
      getRemainingTimeInMillis: () => {
        const timeLeft = executionTimeout - _perf_hooks.performance.now();
